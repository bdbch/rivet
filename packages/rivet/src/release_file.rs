...
impl std::str::FromStr for BumpType {
  ...
  fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
    match s.trim().to_lowercase().as_str() {
      ...
      other => Err(RivetError::ReleaseFile(format!(
...
pub fn parse_release_file(path: &Path) -> Result<ReleaseFile> {
  let content = fs::read_to_string(path)
    .map_err(|e| RivetError::ReleaseFile(format!("Failed to read {}: {}", path.display(), e)))?;
...
pub fn parse_release_content(content: &str, source: &Path) -> Result<ReleaseFile> {
  ...
  if trimmed.is_empty() {
    return Err(RivetError::ReleaseFile(format!(
  ...
  for (key, value) in mapping {
    ...
    let bump_type = bump_str
      .parse::<BumpType>()
      .map_err(|e| RivetError::ReleaseFile(format!("{} in {}", e, source.display())))?;
...
pub fn create_release_file(
  ...
  fs::create_dir_all(release_dir).map_err(RivetError::Io)?;
  ...
  fs::write(&path, content).map_err(RivetError::Io)?;
  Ok(path)
}
...
pub fn archive_release_file(path: &Path, archive_dir: &Path) -> Result<()> {
  fs::create_dir_all(archive_dir).map_err(RivetError::Io)?;
  let filename = path.file_name().ok_or_else(|| {
    RivetError::ReleaseFile(format!("Invalid release file path: {}", path.display()))
  })?;
