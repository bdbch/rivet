//! `rivet pre` — pre-release mode management.
//!
...
use crate::config::{PreModeEntry, RivetConfig};
...
pub fn cmd_pre_enter(tag: &str, package_patterns: &[String], force: bool) -> Result<()> {
  ...
  let (mut config, config_path) = RivetConfig::load(&root)?;
  if config_path.as_os_str().is_empty() {
    return Err(RivetError::Config(
      "No rivet.json found. Run `rivet init` first.".to_string(),
    ));
  }
  ...
  RivetConfig::write_to(&config_path, &config, true)?;
...
pub fn cmd_pre_exit(package_patterns: &[String]) -> Result<()> {
  ...
  let (mut config, config_path) = RivetConfig::load(&root)?;
  if config_path.as_os_str().is_empty() {
    return Err(RivetError::Config("No rivet.json found.".to_string()));
  }
...
pub fn cmd_pre_interactive() -> Result<()> {
  ...
  if selected.is_empty() {
    return Err(RivetError::Other("No packages selected.".to_string()));
  }
