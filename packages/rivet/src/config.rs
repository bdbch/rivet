...
pub(crate) const CONFIG_FILE_NAMES: &[&str] = &[".rivet/config.json", "rivet.json", ".rivet.json"];
  ...
  pub fn load(start_dir: &Path) -> Result<(Self, PathBuf)> {
    ...
    while let Some(dir) = current {
      for name in CONFIG_FILE_NAMES {
        let path = dir.join(name);
        if path.exists() {
          let content = std::fs::read_to_string(&path).map_err(RivetError::Io)?;
          let config: RivetConfig = serde_json::from_str(&content)?;
...
mod tests {
  ...
  fn test_default_config() {
    let config = RivetConfig::default();
    assert_eq!(config.release_dir, ".rivet");
  ...
  fn test_load_no_config_returns_defaults() {
    ...
    let (config, path) = RivetConfig::load(tmp.path()).unwrap();
    assert_eq!(config.release_dir, ".rivet");
  ...
  fn test_write_and_load_config() {
    let tmp = TempDir::new().unwrap();
    let config_path = tmp.path().join("rivet.json");
    let config = RivetConfig {
      release_dir: ".myrelease".to_string(),
      ..Default::default()
    };
    RivetConfig::write_to(&config_path, &config, false).unwrap();

    let (loaded, _) = RivetConfig::load(tmp.path()).unwrap();
