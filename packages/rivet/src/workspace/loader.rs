...
pub fn load_workspace(root: &Path) -> Result<Workspace> {
  ...
  let root_pkg_content = std::fs::read_to_string(&root_pkg_path)
    .map_err(|e| RivetError::Workspace(format!("Failed to read root package.json: {}", e)))?;
...
fn get_workspace_globs(root: &Path) -> Result<Vec<String>> {
  ...
  if pnpm_yaml.exists() {
    let content = std::fs::read_to_string(&pnpm_yaml).map_err(|e| {
      RivetError::Workspace(format!(
        "pnpm-workspace.yaml exists but cannot be read: {}",
        e
      ))
    })?;
    let yaml: serde_yaml::Value = serde_yaml::from_str(&content)
      .map_err(|e| RivetError::Workspace(format!("pnpm-workspace.yaml has invalid YAML: {}", e)))?;
