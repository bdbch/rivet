...
pub fn apply_release_plan(
  ...
  for (_name, bump) in &plan.bumps {
    let pkg = workspace
      ...
      .ok_or_else(|| RivetError::Bump(format!("Package '{}' not found", bump.package_name)))?;
    ...
    if config.sync_cargo_toml {
      ...
      if cargo_path.exists() {
        let cargo_content = std::fs::read_to_string(&cargo_path)
          .map_err(|e| RivetError::Bump(format!("Failed to read Cargo.toml: {}", e)))?;
        ...
        std::fs::write(&cargo_path, new_cargo)
          .map_err(|e| RivetError::Bump(format!("Failed to write Cargo.toml: {}", e)))?;
  ...
  // Save the release manifest for `rivet release`
