...
impl ReleaseManifest {
  ...
  pub fn load(release_dir: &Path) -> Result<Self> {
    let path = Self::path(release_dir);
    if !path.exists() {
      return Err(RivetError::Bump(format!(
        "No release plan found at {}. Run `rivet bump` first.",
        path.display()
      )));
    }
    let content = std::fs::read_to_string(&path)
      .map_err(|e| RivetError::Bump(format!("Failed to read release plan: {}", e)))?;
  ...
  pub fn remove(release_dir: &Path) -> Result<()> {
    let path = Self::path(release_dir);
    if path.exists() {
      std::fs::remove_file(&path).map_err(RivetError::Io)?;
    }
    Ok(())
  }
}
...
fn check_version_exists(package_name: &str, version: &str) -> Result<bool> {
  let output = std::process::Command::new("npm")
    ...
    .output()
    .map_err(|e| RivetError::Bump(format!("Failed to run npm view: {}", e)))?;
...
fn get_access(pkg_json: &PackageJson, config: &RivetConfig) -> String {
...
pub fn publish_manifest(
  ...
  for package_name in &manifest.packages {
    ...
    let output = cmd
      .output()
      .map_err(|e| RivetError::Bump(format!("Failed to run npm publish: {}", e)))?;
...
mod tests {
  ...
  fn test_manifest_roundtrip() {
    ...
    let release_dir = tmp.path().join(".rivet");
