...
use rivet::bump::{apply_release_plan, build_release_plan, find_release_files, print_plan};
use rivet::config::{Access, InternalDepUpdate, PreModeEntry, RivetConfig};
use rivet::release_file::{BumpType, create_release_file};
use rivet::workspace::PackageJson;
use rivet::workspace::load_workspace;
use semver::Version;
use std::path::Path;
...
fn create_config(release_dir: &Path) -> RivetConfig {
  ...
  RivetConfig::write_to(&config_path, &config, true).unwrap();
  config
}
...
fn assert_not_bumped(plan: &rivet::bump::ReleasePlan, pkg_name: &str) {
  assert!(
...
fn test_full_user_flow_init_pre_mode_bump() {
  ...
  let config = create_config(&rivet_dir);
  assert_eq!(config.linked[0].len(), 2);
  assert_eq!(config.fixed.len(), 1);
  assert_eq!(config.pre_mode.len(), 2);

  // ── Phase 3: Create release files ──
  create_releases(&rivet_dir);
  assert_eq!(find_release_files(&rivet_dir).unwrap().len(), 6);

  // ── Phase 4: Build plan & verify every bump ──
  println!("\n--- Status (before bump) ---");
  let plan = build_release_plan(&workspace, &config, &rivet_dir).unwrap();
  print_plan(&plan);
  ...
  println!("\n--- Dry-run bump ---");
  apply_release_plan(&workspace, &plan, &config, &rivet_dir, true, false).unwrap();
  ...
  // ── Phase 6: Real bump ──
  println!("\n--- Real bump ---");
  apply_release_plan(&workspace, &plan, &config, &rivet_dir, false, false).unwrap();
  ...
  assert_version(&tmp.path().join("packages/utils"), "1.0.0");
  assert_eq!(
    find_release_files(&rivet_dir).unwrap().len(),
    0,
  ...
  create_release_file(&rivet_dir, &r6, "Second internal fix", None).unwrap();

  let workspace2 = load_workspace(tmp.path()).unwrap();
  let plan2 = build_release_plan(&workspace2, &config, &rivet_dir).unwrap();
  ...
  apply_release_plan(&workspace2, &plan2, &config, &rivet_dir, false, false).unwrap();
  assert_version(&tmp.path().join("packages/internal"), "1.1.0-alpha.2");

  let pre_state = rivet::prerelease::PreState::load(&rivet_dir).unwrap();
  assert_eq!(
...
// ---------------------------------------------------------------------------
// Solo repo changelog test — verifies that `rivet bump` in a non-monorepo
...
fn create_solo_config(release_dir: &Path) -> RivetConfig {
  let config_path = release_dir.join("config.json");
...
fn test_solo_repo_always_creates_changelog() {
  ...
  let rivet_dir = tmp.path().join(".rivet");
  let config = create_solo_config(&rivet_dir);
  ...
  create_release_file(&rivet_dir, &r1, "Fix login bug", None).unwrap();
  assert_eq!(find_release_files(&rivet_dir).unwrap().len(), 1);
...
fn test_solo_repo_changelog_disabled_when_changelog_false() {
  ...
  let rivet_dir = tmp.path().join(".rivet");
  let config_path = rivet_dir.join("config.json");
  let config = RivetConfig {
    changelog: false,
    generate_packages_changelog: false,
    generate_global_changelog: false,
