...
use crate::config::{OxrlsConfig, find_existing_config};
...
mod tests {
  ...
  fn test_init_returns_early_when_config_exists() {
    ...
    let config_dir = dir.path().join(".oxrls");
  ...
  fn test_init_creates_config_when_none_exists() {
    ...
    let config_path = dir.path().join(".oxrls").join("config.json");
    assert!(config_path.exists());

    let release_dir = dir.path().join(".oxrls");
  ...
  fn test_find_existing_config_finds_all_three_names() {
    let cases: [(&str, fn(&std::path::Path) -> std::path::PathBuf); 3] = [
      (".oxrls/config.json", |p| {
        ...
        p.join(".oxrls").join("config.json")
      }),
      ("oxrls.json", |p| {
        fs::write(p.join("oxrls.json"), "{}").unwrap();
        p.join("oxrls.json")
      }),
      (".oxrls.json", |p| {
        fs::write(p.join(".oxrls.json"), "{}").unwrap();
