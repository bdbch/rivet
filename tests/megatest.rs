...
fn write_pkg(dir: &Path, name: &str, version: &str, deps: Option<&[(&str, &str)]>) {
    ...
    if let Some(d) = deps {
        let dep_map: serde_json::Map<String, serde_json::Value> = d
            ...
            .collect();
        map.insert("dependencies".to_string(), serde_json::Value::Object(dep_map));
    }
...
fn scaffold_workspace(tmp: &TempDir) {
    ...
    root_map.insert("name".to_string(), serde_json::Value::String("root".to_string()));
    root_map.insert("version".to_string(), serde_json::Value::String("1.0.0".to_string()));
    ...
    write_pkg(&root.join("packages/internal"), "@scope/internal", "1.0.0", None);
...
fn assert_version(dir: &Path, expected: &str) {
    ...
    assert_eq!(pkg.version.as_deref(), Some(expected), "{} has wrong version", dir.display());
}

fn assert_not_bumped(plan: &oxrls::bump::ReleasePlan, pkg_name: &str) {
    assert!(!plan.bumps.contains_key(pkg_name), "{pkg_name} should NOT be bumped");
}
...
fn test_full_user_flow_init_pre_mode_bump() {
    ...
    for name in &[
        "@scope/core", "@scope/utils", "@scope/ui", "@scope/icons",
        "@scope/app", "@scope/web", "@scope/internal", "@scope/tools",
    ] {
    ...
    assert_eq!(icons.bump_type, BumpType::Minor, "Linked group should elevate patch→minor");
    ...
    assert_eq!(web.new_version, app.new_version, "Fixed group must share version");
    ...
    assert_eq!(internal.new_version, Version::parse("1.1.0-alpha.1").unwrap());
    ...
    assert_version(&tmp.path().join("packages/utils"), "1.0.0");
    assert_eq!(find_release_files(&oxrls_dir).unwrap().len(), 0, "Release files consumed");
    ...
    assert_eq!(internal2.old_version, Version::parse("1.1.0-alpha.1").unwrap());
    // Subsequent pre-release: base stays 1.1.0, counter → 2
    assert_eq!(internal2.new_version, Version::parse("1.1.0-alpha.2").unwrap());
