---
title: Configuration
description: All configuration options for oxrls
---

oxrls looks for configuration in the following locations (in order of priority):

1. `.oxrls/config.json` — in the project root or any parent directory
2. `oxrls.json` — in the project root or any parent directory
3. `.oxrls.json` — hidden file variant

If no config file is found, oxrls uses sensible defaults.
...
When enabled, oxrls also bumps the version in `Cargo.toml` files that are found alongside `package.json`. This is useful for Rust projects that also publish npm packages (e.g., napi-rs projects).
...
This is a unique oxrls feature — it sits between "completely independent" and "always the same version" (fixed groups).
